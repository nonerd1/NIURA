...
const HomeScreen = () => {
  ...
  const [isEarbudsConnected, setIsEarbudsConnected] = useState(false);
  const [mentalReadiness, setMentalReadiness] = useState<number>(78);
  ...
  */
  ...
  // Update the useEffect that handles focus and stress values
  useEffect(() => {
    // Priority order for values:
    // 1. Demo mode (highest priority)
    // 2. BLE values (if connected)
    // 3. Random generated values (fallback)
    
    if (demoMode) {
      // If in demo mode, use demo values
      setFocusValue(demoFocusValue);
      setStressValue(demoStressValue);
    } else if (isConnected) {
      // If BLE is connected, use BLE values
      ...
      setFocusData(prev => {
        const newData = [...prev.data, bleFocusValue].slice(-6); // Keep last 6 points
        const newLabels = [...prev.labels, timestamp].slice(-6);
        return { data: newData, labels: newLabels };
      });
      
      setStressData(prev => {
        const newData = [...prev.data, bleStressValue].slice(-6); // Keep last 6 points
        const newLabels = [...prev.labels, timestamp].slice(-6);
        return { data: newData, labels: newLabels };
      });
    } else {
      // Original random value generation logic for when not in demo mode and not connected to BLE
      const interval = setInterval(() => {
        // Generate slightly random values
        const newFocusValue = Math.max(0.5, Math.min(2.8, focusValue + (Math.random() * 0.4 - 0.2)));
        const newStressValue = Math.max(0.5, Math.min(2.8, stressValue + (Math.random() * 0.4 - 0.2)));
        
        setFocusValue(newFocusValue);
        setStressValue(newStressValue);
        
        // Add to history for graphs
        const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        
        setFocusData(prev => {
          const newData = [...prev.data, newFocusValue].slice(-6); // Keep last 6 points
          const newLabels = [...prev.labels, timestamp].slice(-6);
          return { data: newData, labels: newLabels };
        });
        
        setStressData(prev => {
          const newData = [...prev.data, newStressValue].slice(-6); // Keep last 6 points
          const newLabels = [...prev.labels, timestamp].slice(-6);
          return { data: newData, labels: newLabels };
        });
        
        // Random readiness
        setMentalReadiness(Math.floor(60 + Math.random() * 40));
      }, 5000);
      
      return () => clearInterval(interval);
    }
  }, [demoMode, demoFocusValue, demoStressValue, isConnected, bleFocusValue, bleStressValue]);

  // Add a BLE connection indicator and buttons to the dashboard section
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginRight: 16 }}>
        ...
        {isConnected && (
          <View style={styles.statusIndicator}>
            ...
            <Text style={styles.statusText}>ESP32 Connected</Text>
          </View>
        )}
        
        {bleError && (
          <View style={styles.statusIndicator}>
            <MaterialCommunityIcons name="bluetooth-off" size={18} color="#F44336" />
  ...
  // Add BLE connect/disconnect buttons to the dashboard
    ...
    return (
      <View style={styles.bleControlsContainer}>
        {!isConnected ? (
          ...
          <Pressable
            style={[styles.bleButton, styles.bleDisconnectButton]}
          ...
          >
            <MaterialCommunityIcons
              ...
              color="#FFF"
            />
            <Text style={styles.bleButtonText}>Disconnect ESP32</Text>
          </Pressable>
        )}
